...
// Helpers for Degree and Salary display transformations
function get_degree_years_label($degree_raw) {
    $deg = strtolower(trim((string)$degree_raw));
    if (empty($deg)) return '';

    if (strpos($deg, 'certificate') !== false || strpos($deg, 'high school') !== false) {
        return 'Certificate (1–2 years)';
    } elseif (strpos($deg, 'associate') !== false) {
        return 'Associate (2–3 years)';
    } elseif (strpos($deg, 'bachelor') !== false) {
        return 'Bachelor\'s (4–5 years)';
    } elseif (strpos($deg, 'master') !== false || strpos($deg, 'doctoral') !== false || strpos($deg, 'graduate') !== false) {
        return 'Graduate (6+ years)';
    }

    return htmlspecialchars($degree_raw);
}

function get_salary_tier_badge($salary_raw) {
    $sal = strtolower(trim((string)$salary_raw));
    if (empty($sal)) return ['signs' => '', 'class' => 'badge-salary', 'title' => ''];

    // Tier 1 represents top earnings, scaling down to lower tiers
    if (strpos($sal, 'tier 1') !== false || $sal === '$$$$') {
        return ['signs' => '$$$$ (Premium)', 'class' => 'badge-salary badge-salary-tier4', 'title' => 'High Earning Salary'];
    } elseif (strpos($sal, 'tier 2') !== false || $sal === '$$$') {
        return ['signs' => '$$$ (Comfortable)', 'class' => 'badge-salary badge-salary-tier3', 'title' => 'Comfortable Salary'];
    } elseif (strpos($sal, 'tier 3') !== false || $sal === '$$') {
        return ['signs' => '$$ (Starting Out)', 'class' => 'badge-salary badge-salary-tier1', 'title' => 'Starting Out Salary'];
    }

    return ['signs' => htmlspecialchars($salary_raw), 'class' => 'badge-salary', 'title' => 'Salary'];
}
?>
<!DOCTYPE html>
<html lang="en">
                                ...
                                <div class="career-info" style="flex-grow: 1;">
                                    ...
                                    <div class="career-meta">
                                        <?php if (!empty($career['degree_required'])): ?>
                                            <span class="badge badge-degree">Entry requirement: <?php echo get_degree_years_label($career['degree_required']); ?></span>
                                        <?php endif; ?>
                                        <?php 
                                        if (!empty($career['median_salary'])): 
                                            $salary_info = get_salary_tier_badge($career['median_salary']);
                                        ?>
                                            <span class="badge <?php echo $salary_info['class']; ?>" title="<?php echo $salary_info['title']; ?>">
                                                Salary: <strong><?php echo $salary_info['signs']; ?></strong>
                                            </span>
                                        <?php endif; ?>
                                        <?php if (!empty($career['growth_rate'])): ?>
                                            <span class="badge badge-growth">Demand: <?php echo htmlspecialchars($career['growth_rate']); ?></span>
                                        <?php endif; ?>
                                    </div>
