...
// Ensure applicant data exists[cite: 8]
if (!isset($_SESSION['applicant'])) {
    header('Location: index.php');
    exit;
}

// Handle Form Submission for Page 2 with Dynamic Routing and SQL Persistence[cite: 8]
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'step_2') {
    ...
    // Save LCP selection in SQL
    $stmt = $pdo->prepare("
        UPDATE applicants 
        SET lcp = :lcp, last_updated = :last_updated 
        WHERE email = :email
    ");
    $stmt->execute([
        'lcp' => $selected_pathway,
