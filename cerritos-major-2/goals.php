...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_goals') {
    ...
    // Save/Update Goals and Priority via SQL
    $stmt = $pdo->prepare("
        INSERT INTO applicant_goals (email, top_priority, last_completed) 
        VALUES (:email, :top_priority, :last_completed)
    ...
    // Update main applicant record updated timestamp
    $stmt = $pdo->prepare("UPDATE applicants SET last_updated = :last_updated WHERE email = :email");
    $stmt->execute([
