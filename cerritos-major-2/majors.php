...
// Fetch degree & program listings from database table `programs`
$degrees_data = [];
try {
    $stmt = $pdo->query("SELECT program_name, credential_type, credential_category, description, related_career_matches FROM programs");
    while ($row = $stmt->fetch()) {
        $row['related_career_matches'] = json_decode($row['related_career_matches'] ?? '[]', true);
        $degrees_data[] = $row;
    }
} catch (\PDOException $e) {
    // Fallback if degrees.json file is used
    if (file_exists('degrees.json')) {
        $degrees_data = json_decode(file_get_contents('degrees.json'), true) ?: [];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_major') {
    ...
    if (!empty($selected_major_key)) {
        ...
        $stmt = $pdo->prepare("
            UPDATE applicants 
            SET selected_major = :selected_major, selected_credential_type = :credential_type, last_updated = :last_updated 
            WHERE email = :email
        ");
        $stmt->execute([
...
foreach ($degrees_data as $data) {
    if (isset($data['program_name']) && isset($data['related_career_matches']) && is_array($data['related_career_matches'])) {
        ...
        foreach ($data['related_career_matches'] as $rcm) {
            ...
            if (!empty($rcm_title) && in_array($rcm_title, $selected_careers, true)) {
                $matched_programs[] = [
