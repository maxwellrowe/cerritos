...
$xpath = "/rss/channel/item[cal:tags/cal:tag='SC Writing Workshops']";
...
function displayCalendarListing($y){
    ...
    $start_date = dateConv($date, "l, F d, Y");
    ...
    if($start_date >= $today_date) {
        ...
        //$link = $y->link;
		$link = $y->children($ns['cal'])->ticketUrl;
        $title = $y->title;		
        $end = $y->children($ns['cal'])->end;   
        $start = "";
        ...
        $res .= '<h3 class="dailyFalcon"><a href="'.$link.'">'.$start_date.'</a></h3> 
