                            ...
                            <div id="contentcontainer">
								...
								<xsl:choose>
									<xsl:when test="$ou:action = 'pub'">
										<div id="DFEventsDiv"></div>
										<xsl:variable name="formatted-feed-url" select="replace(/document/ouc:properties[@label='config']/parameter[@name='full-rss-feed'], '&amp;', '%26')" />
										<script>
											$("#DFEventsDiv").load("/_resources/php/wnl/daily-falcon-today-new.php?feed=<xsl:value-of select="$formatted-feed-url" />&amp;num_items=50");
										</script><noscript>The calendar requires javascript enabled.</noscript>
									</xsl:when>
									<xsl:otherwise>
										<div id="DFEventsDiv">Events will display on production.</div>
									</xsl:otherwise>
								</xsl:choose>
