	...
	<div class="ItemIndexHeading"><a name="{$seq}"><xsl:value-of select="$seq"/></a></div>
      <ul class="AbcItem">
		...
		<!-- End new setup -->
		  ...
		  <xsl:variable name = "emailaddr" select="lower-case(emailaddr[text()])" />
          <xsl:if test="substring(lower-case($lastname),1,1)=lower-case($seq)">
            <li>
				<xsl:choose>
					<xsl:when test="$emailaddr != '' and substring-before(lower-case($emailaddr),'@cerritos.edu') != 'noreply'">
						<!-- link to the detail page -->
						<a href="{substring-before(lower-case($emailaddr),'@cerritos.edu')}.htm"><xsl:value-of select="lastname"/>, <xsl:value-of select="firstname"/><xsl:if test="$middlename != ''">&#160;<xsl:value-of select="$middlename"/></xsl:if></a>
					</xsl:when>
					<xsl:otherwise>
						<!-- show the full listing unlinked -->
						<span class="indent">
							<xsl:choose>
						...
						<xsl:value-of select="jobtitle"/>,&#160;<xsl:value-of select="divdept"/> 
