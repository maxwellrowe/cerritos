...
<xsl:template match="/">
	...
	<!-- changes class in h2 heading based on page type, department home or inside. dept inside page requires 2.5em top margin -->
...
<div class="clearfix">
	<!-- choose layout, 2 column or full width -->
	<xsl:choose>
	...
	<xsl:otherwise>
        <div id="maincontent" class="{normalize-space($MaincontentClass)}">
            ...
            <div id="contentcontainer">
				<!-- heading -->
                <h2 class="{normalize-space($HeadingClass)}"><xsl:value-of select="document/ouc:properties[@label='metadata']/title" disable-output-escaping="yes" /></h2>
				<!-- slider -->
				<xsl:apply-templates select="document/ouc:div[@label='deptslider']" />
                <!-- dept intro -->
                <h4><xsl:apply-templates select="document/ouc:div[@label='deptintro']" /></h4>
