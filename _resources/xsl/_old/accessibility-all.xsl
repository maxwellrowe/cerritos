...
<!-- tb 
	sets the popup/document icons and classes in prv and pub modes
	all other markup only shows in edt mode
	this adds info for h1-6
	this handles .ua-comment class for pdf docs fixed
}
-->
	
	<!-- ################# links ################# -->
	<xsl:template match="a">	
		<xsl:variable name="vBaseUri" select="'http://www.cerritos.edu/'"/>
		<!-- link icons -->
		<xsl:if test="$ou:action !='edt'">
		<xsl:choose>
			<!-- exclude links here that contain youtube.com, etc. <xsl:if test=".[(//img/* != '') and not(contains(lower-case(@src),'youtube.com'))]"> -->
		<xsl:when test=".[not (contains(lower-case(@href), 'youtube.com')) and not(contains(lower-case(@href),'cerritoscollege.granicus.com')) and not(contains(lower-case(@href),'textcast.peoplesupport.com')) and not(contains(lower-case(@onclick), 'youtube.com')) and not(contains(lower-case(@onclick), 'youtu.be')) and not(contains(lower-case(@href), 'youtu.be'))]">
				...
				<xsl:when test=".[(starts-with(@href, 'http')) and not(contains(@href, 'cerritos.edu'))]">
					...
					<!-- no icons for image links -->
					<span class="new-window"><img src="{{f:80150929}}" alt="new window" /></span>
				</xsl:when>
				...
				<xsl:otherwise>
					...
					<xsl:choose>
						...
						<xsl:when test=".[contains(lower-case(@href), 'mailto:')]">
							...
							<!-- icon  -->
							<span class="mailto"><img src="{{f:80150919}}" alt="email link" /></span>
						</xsl:when>
						<!-- pdf file icon -->
						<xsl:when test=".[contains(lower-case(@href), '.pdf')]">
							...
							<span class="new-window"><img src="{{f:80150933}}" alt="PDF document" /></span>
						</xsl:when>
						<!-- word doc file icon -->
					...
					<!-- end file icons for local web server -->
				</xsl:otherwise>
				</xsl:choose>
		</xsl:when>
		...
		<xsl:copy-of select="."/>
			...
			<!-- pdf file old warning not repaired -->
			<xsl:if test=".[(contains(lower-case(@href),'.pdf') and contains(lower-case(@href),'/uploads/') and not(contains(lower-case(@href),'_ua.pdf')))]">
