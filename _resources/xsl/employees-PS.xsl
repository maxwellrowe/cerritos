	...
	<div class="ItemIndexHeading"><a name="{$seq}"><xsl:value-of select="$seq"/></a></div>
      ...
      <ul class="EmplItem">		
		...
		<xsl:variable name="Employee_File">
			<xsl:if test="doc-available(concat('file:', $ou:root, $ou:site, '/_resources/xml/EMPL_DIR140126-102213.xml'))">
				<xsl:copy-of select="doc(concat('file:', $ou:root, $ou:site, '/_resources/xml/EMPL_DIR140126-102213.xml'))"/>
			</xsl:if>
		</xsl:variable>
		...
		<!-- End new setup -->
          ...
          <xsl:variable name = "ext" select="EXTENSION[text()]" />
		  <xsl:variable name = "emailaddr" select="emailaddr[text()]" />
		  <xsl:variable name="tokenizedLastName" select="
    string-join(
        for $x in tokenize($lastname, '\s+') 
        return string-join(
            for $y in tokenize($x, '-') 
            return 
                let $parts := tokenize($y, '''')  (: Split by apostrophe :)
                return 
                    string-join(
                        for $i in $parts 
                        return 
                            if (position() = 1) then
                                concat(
                                    upper-case(substring($i, 1, 1)), 
                                    lower-case(substring($i, 2))
                                )
                            else
                                concat(
                                    upper-case(substring($i, 1, 1)), 
                        ...
                        ''''  (: Join parts back with apostrophe :)
                    ), 
            '-'
        ), 
        ' '
...
"/>
					...
					<xsl:variable name="tokenizedFirstName" select="string-join(for $x in tokenize($firstname,'\s+') return string-join(for $y in tokenize($x, '-')return concat(upper-case(substring($y, 1, 1)), lower-case(substring($y, 2))), '-'),' ')" />
					
          <xsl:if test="substring($lastname,1,1)=$seq">			  
            <li>				  
				  <strong><xsl:value-of select="$tokenizedLastName" /></strong>, <strong><xsl:value-of select="$tokenizedFirstName"/></strong>, <xsl:value-of select="jobtitle"/>, <xsl:value-of select="divdept"/> 
               <xsl:if test="$ext != ''">, ext. <xsl:value-of select="$ext"/></xsl:if>
				<xsl:if test="$emailaddr != '' and substring-before(lower-case($emailaddr),'@cerritos.edu') != 'noreply'">, <a href="/scripts/employeesmailform-2.aspx?e={ou:parseString(substring-before($emailaddr,'@cerritos.edu'))}&amp;n={ou:parseString(($firstname))}&amp;l={ou:parseString(($lastname))}">
					email <xsl:value-of select="$tokenizedFirstName"/>&#160;<xsl:value-of select="$tokenizedLastName"/>
					</a></xsl:if>			
