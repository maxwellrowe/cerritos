...
<div>
<form id="ContactForm" action="{{f:80119383}}" enctype="multipart/form-data" method="post" name="ContactForm">
<p>
	<!-- EMAIL RECIPIENTS==YOU MUST CHANGE THESE -->
	<input name="_recipients" type="hidden" value="{ou:processEmails($Recipient1,$Recipient2,$Recipient3,$Recipient4,$Recipient5,$Recipient6)}" /> 
	
	<!-- REQUIRED FIELDS -->
	<input name="_requiredFields" type="hidden" value="Name,Email,Request" /> 
	...
	<input name="_redirect" type="hidden" value="{$ConfirmationPage}" />
</p>
	...
	<p>
		<label for="Request"><strong>* Request:</strong></label><br />
		<textarea id="Request" cols="40" rows="10" name="Request" required="true"></textarea>
	</p>
	
	<p>
		<input type="file" name="FileUpload1" ID="FileUpload1" multiple="multiple"/>
<small>You can select multiple files by holding Ctrl (Windows) or Cmd (Mac).</small>
	</p>
	...
	<div class="g-recaptcha" data-sitekey="6LcxUk4UAAAAAMyg221MHklN6E_axt_jNmxZ69fI"></div><br /> 
	<input id="Submit" type="submit" value="Submit" /> 
	<label for="Reset" style="border: 0; clip: rect(0 0 0 0);height: 1px;margin: -1px;overflow: hidden;padding: 0;position: absolute;width: 1px;">Reset</label>
	<input id="Reset" type="reset" value="Reset" /> *Required Fields
