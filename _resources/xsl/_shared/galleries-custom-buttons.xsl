...
<xsl:stylesheet version="3.0" 
	...
	<xsl:template match="gallery[$gallery-type = 'slick-fade']" priority="2">
	...
	<xsl:template match="gallery[$gallery-type = 'slick-custom-button']" priority="2">
		<div class="slick-custom-button">
			<xsl:for-each select="images/image">
				<div>
					<xsl:choose>
						<!-- Determine if caption field is empty -->
						<xsl:when test="link != ''">
							<!-- Not empty, create a link -->
							<img src="{@url}" alt="{description}" title="{title}" />
							<div class="slick-caption"><a class="button" href="{link}"><xsl:value-of select="(title)" /></a></div>
						</xsl:when>
						<xsl:otherwise>
							<img src="{@url}" alt="{description}" title="{title}" />
						</xsl:otherwise>
		...
		<div class="buttons"><a href="{advanced}">d</a>&nbsp;<button id="toggle">Pause</button></div>
	</xsl:template>
	...
	<xsl:template name="gallery-headcode">
		...
		<xsl:if test="/document/descendant::gallery">
			<xsl:choose>
				...
				<xsl:when test="$gallery-type = 'slick-fade'">
					<link rel="stylesheet" href="{$domain}/_resources/ldp/galleries/slick-2/slick.css" type="text/css"/>
					<link rel="stylesheet" href="{$domain}/_resources/ldp/galleries/slick-2/slick-theme.css" type="text/css"/>
				</xsl:when>
				...
				<xsl:when test="$gallery-type = 'slick-custom-button'">
					<link rel="stylesheet" href="{$domain}/_resources/ldp/galleries/slick-custom-button/slick.css" type="text/css"/>
					<link rel="stylesheet" href="{$domain}/_resources/ldp/galleries/slick-custom-button/slick-theme.css" type="text/css"/>
					<!--<link rel="stylesheet" href="{$domain}/_resources/css/components.css" type="text/css"/>-->
				</xsl:when>
	...
	<xsl:template name="gallery-footcode">
		...
		<xsl:if test="/document/descendant::gallery">
			<xsl:choose>
				...
				<xsl:when test="$gallery-type = 'slick-fade'">
					<script type="text/javascript" src="{$domain}/_resources/ldp/galleries/slick-2/slick.min.js"></script>
					<script type="text/javascript">
						$(document).ready(function() {
							$('.slick-fade').slick({
								dots: true,
				...
				<xsl:when test="$gallery-type = 'slick-custom-button'">
					<script type="text/javascript" src="{$domain}/_resources/ldp/galleries/slick-custom-button/slick.min.js"></script>
					<script type="text/javascript">
						$(document).ready(function() {
							$('.slick-custom-button').slick({
								infinite: false,
								slidesToShow: 1,
								accessibility: true,
								pauseOnHover: false,
							...
							$('#toggle').click( function() {
								...
								if ($(this).html() == 'Play'){
     								$('.slick-custom-button').slick('slickPlay')
    								 $(this).html('Pause') 
  								} else {
   									 $('.slick-custom-button').slick('slickPause')  
    								$(this).html('Play') 
  								} 
