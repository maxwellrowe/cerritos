...
<xsl:stylesheet version="3.0" 
	...
	<xsl:function name="ou:find-up-include" as="xs:string">
	  ...
	  <xsl:variable name="candidate" select="concat($dir, '_includes/assets/', $filename)" />
	  <xsl:variable name="probe"
		select="if ($global) then concat($ou:www-domain, $candidate) else concat($domain, $candidate)" />
