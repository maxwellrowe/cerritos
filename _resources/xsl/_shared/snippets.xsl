...
<xsl:stylesheet version="3.0"
	...
	<!-- Snippets
	1. Two Column Flex
	2. Slick Slide Gallery
	3. Flexslider Gallery
	4. Title Bar
	5. Announcements
	6. Events
	7. Accordion
	8. Feature Box Icon Title Text
	9. Light Blue Well
	10. Dark Blue Well
	11. Dark Blue Heading
	12. Two Column Layout
	13. Two Column 8-4 Layout
	14. Two Column 8-4 Reverse Mobile Layout
	15. Two Column 3-9 Layout
	16. Three Column Layout
	17. Three Column Wide Center Layout
	18. Four Column Layout
	19. Six Column Layout
	20. Tabs
	21. Calendar Boxes
	22. OU Calendar
	...
	<!-- Non-Snippet Template Matches And Utilities -->
	...
	<!-- Utility: Insert Line Breaks Between Link Words -->
...
<xsl:template match="table[contains(@class, 'ou-container-events')]">
	...
	<xsl:template match="table[contains(@class, 'ou-accordion')]">
		...
		<xsl:variable name="size" select="replace(normalize-space(thead/tr/th[@class='ou-accordion-size']),'&nbsp;','')" />
		<xsl:variable name="style" select="replace(normalize-space(thead/tr/th[@class='ou-accordion-style']),'&nbsp;','')" />
		<div id="{$accordion-id}" class="accordion accordion-size-{$size} accordion-style-{$style}">
	...
	<!-- Snippet 8: Feature Box Icon Title Text -->
...
<xsl:template match="table[contains(@class, 'ou-dark-blue-well')]">
	...
	<!-- Snippet 11: Dark Blue Heading -->
<xsl:template match="table[contains(@class, 'ou-dark-blue-heading')]">
		<div class="p-3 bg-primary mb-3 rounded-2">
			<h2 class="h3 text-white m-0">
				<span>
					<xsl:apply-templates select="tbody/tr[@class = 'ou-content']/td[contains(@class, 'ou-text')]/node()" />
				</span>
			</h2>
...
<xsl:template match="table[contains(@class, 'ou-2-column-layout')]">
	...
	<!-- Snippet 13: Two Column 8-4 Layout -->
	...
	<!-- Snippet 14: Two Column 8-4 Reverse Mobile Layout -->
	...
	<!-- Snippet 15: Two Column 3-9 Layout -->
	...
	<!-- Snippet 16: Three Column Layout -->
<xsl:template match="table[contains(@class, 'ou-3-column-layout')]">
	...
	<!-- Snippet 17: Three Column Wide Center Layout -->
...
<xsl:template match="table[@class='ou-tabs']">
	...
	<ul class="nav nav-tabs" id="{concat('tabs-', $unique)}" role="tablist">
		<xsl:for-each select="tbody/tr">
			<xsl:variable name="title" select="td[@class='title']" />
		<li class="nav-item" role="presentation">
			<button
				class="nav-link{if (position()=1) then ' active' else ''}"
				id="{concat('tab-', position(), '-', $unique, '-tab')}"
				type="button"
				role="tab"
				data-bs-toggle="tab"
				data-bs-target="#tab{concat(position(),'-',$unique)}"
				aria-controls="tab{concat(position(),'-',$unique)}"
				aria-selected="{if (position()=1) then 'true' else 'false'}"
	...
	<div class="tab-content border border-top-0 p-3">
		<xsl:for-each select="tbody/tr">
			<xsl:variable name="title" select="td[@class='title']" />
		<div
			class="tab-pane fade{if (position()=1) then ' show active' else ''}"
			id="{concat('tab',position(),'-',$unique)}"
			role="tabpanel"
			aria-labelledby="{concat('tab-', position(), '-', $unique, '-tab')}"
			tabindex="0"
		>
	...
	<xsl:template match="table[contains(@class, 'ou-container')]">
		...
		<xsl:variable name="classes" select="replace(normalize-space(tbody/tr/td[@class='ou-container-classes']),'&nbsp;','')" />
		<div class="cerritos-full-width {$classes}" style="background: {$bg};">
	...
	<!-- 24. Layout - Multiple Columns -->
	<xsl:template match="table[contains(@class, 'ou-multiple-columns')]">
		<xsl:variable name="vertical-alignment" select="replace(lower-case(thead/tr/th[@class='vertical-alignment']),'&nbsp;','')" />
		<div class="row row-snippet align-items-{$vertical-alignment}">
			<xsl:for-each select="tbody/tr">
	...
	<!-- 25. Layout - Grid -->
	<xsl:template match="table[contains(@class, 'ou-grid')]">
		<xsl:variable name="mobile" select="replace(lower-case(normalize-space(thead/tr/th[@class='mobile-columns'])),'&nbsp;','')" />
		<xsl:variable name="tablet" select="replace(lower-case(normalize-space(thead/tr/th[@class='tablet-columns'])),'&nbsp;','')" />
		<xsl:variable name="desktop" select="replace(lower-case(normalize-space(thead/tr/th[@class='desktop-columns'])),'&nbsp;','')" />
		
		<div class="row row-cols-{$mobile} row-cols-md-{$tablet} row-cols-lg-{$desktop} g-4">
			<xsl:for-each select="tbody/tr">
				<div class="col">
					<xsl:apply-templates select="td[@class='content']/node()" />
				</div>
