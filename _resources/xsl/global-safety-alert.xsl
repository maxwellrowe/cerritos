		...
		<div id="{$alert-id}" class="{$variant-class} alert {$bootstrap-class} alert-dismissible fade show rounded-0 border-0 w-100 mb-0 py-3 px-4 z-3" role="alert" aria-live="{$live-region}" aria-atomic="true">
			<div class="container-xxl">
				<div class="d-flex align-items-center gap-3">
					...
					<button type="button" class="close-icon btn-close ms-auto flex-shrink-0" aria-label="Dismiss alert"></button>
				</div>
			</div>
		</div>
	</xsl:template>
		...
		<xsl:choose>
			<xsl:when test="document/ouc:properties[@label='config']/parameter[@name='AlertsDiv']/option[@selected='true' and @value='info-white']">
				<xsl:call-template name="render-alert">
					<xsl:with-param name="alert-id" select="'emergencyNotice'" />
