...
$site_uuids = array(
	...
	"www-cerritos-edu" => "5a6d406a-0891-4298-a12f-f0f373181178",
     "ouc-secure-cerritos-edu" => "5d9c8efa-68dc-43f3-9234-6755edd5c610"
);
$debug = true; // enable "debug mode" for this script
...
class ouldp {
	...
	private function createArrayXML($array, &$xml) {
		// Check if the array is associative (has string keys)
		if (array_keys($array) !== range(0, count($array) - 1)) {
			// It's associative, proceed as before
			$structElem = $xml->addChild('struct');
			foreach ($array as $key => $val) {
				$key = htmlspecialchars($key);
				$memberElem = $structElem->addChild('member');
				$memberElem->addChild('name', $key);
				$memberValueElem = $memberElem->addChild('value');
				if (is_array($val)) {
					$this->createArrayXML($val, $memberValueElem);
				} else {
					$val = htmlspecialchars($val);
					$memberValueElem->addChild('string', $val);
				}
			}
		} else {
			// It's a non-associative array, concatenate the values separated by commas
			$val = htmlspecialchars(implode(',', $array));
	...
	public function send($method, $param) {
		...
		//SimpleXML to turn form data into xmlrpc (replacing xmlrpc_encode_request)
		$output_options = ['encoding' => 'utf-8', 'escaping' => 'markup', 'verbosity' => 'no_white_space'];
		//create the XML-RPC request using SimpleXML (encode)
		$request = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><methodCall></methodCall>');
		$request->addChild('methodName', $method);	
		$params = $request->addChild('params');	
		foreach ($param as $value) {	
			$param = $params->addChild('param');	
			$valueElem = $param->addChild('value');	
			if (is_array($value)) {	
				$this->createArrayXML($value, $valueElem);	
			} else {	
				$valueElem->addChild('string', $value);	
			}	
		}	
		// add attributes to the methodCall element	
		$request->addAttribute('encoding', $output_options['encoding']);	
		$request->addAttribute('escaping', $output_options['escaping']);	
		$request->addAttribute('verbosity', $output_options['verbosity']);	
...
// 			curl_close($ch);
			...
			//decode ssmResponse from curl method	
			// Convert the XML object to an array	
			$xml = simplexml_load_string($ssmResponse);	
			$result = json_decode(json_encode($xml), true);	
			$result = $result['params']['param']['value']['struct']['member'];	
			...
			function parseArray($inputArray) {	
				...
				foreach ($inputArray as $item) {	
					...
					switch (key($item['value'])) {	
						case 'boolean':	
						$outputArray[$name] = $item['value']['boolean'] == '1' ? true : false;	
						break;	
						case 'string':	
						$outputArray[$name] = $item['value']['string'];	
						break;	
						case 'array':	
