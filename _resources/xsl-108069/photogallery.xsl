...
<xsl:template match="/">
	...
	<head>		
		...
		<script>
			<![CDATA[
    		function visibility(ID,targetID) {
...
<!--<xsl:with-param name="path" select="$LeftNav"/>-->
	<xsl:with-param name="path" select="if ($LeftNav != '') then $LeftNav else concat($firstdir, '/_includes/assets/deptnav.inc')"/>
</xsl:call-template>
</nav>
</div>
			</div>
				...
				<div id="deptinfo">
					...
					<xsl:call-template name="unparsed-include-file">
						...
						<xsl:with-param name="path" select="if ($DeptInfo != '') then $DeptInfo else concat($firstdir, '/_includes/assets/deptinfo.inc')"/>
					</xsl:call-template>
    			</div>
...
<!-- footer -->
	...
	<xsl:template name="PhotoGallery">
		...
		<xsl:variable name="GalleryFileVar" select="normalize-space(document/ouc:properties[@label='config']/parameter[@name='GalleryFile'])" />
		<!--xsl:value-of select="$GalleryFolderString"/-->
		<!-- thumbnails div -->
		<div id="photoGallery">
			
			<!-- Previous setup. Comment out due to XSLT Processor Upgrade to Saxon 11.3 5/19/22 https://support.moderncampus.com/troubleshooting/saxon-upgrade.html -->
		<!-- <xsl:for-each select="document(concat('file:', $ou:root, $ou:site, $GalleryFileVar))/gallery/photo"> -->

		<!-- New set up due to XSLT Processor Upgrade to Saxon 11.3 5/19/22 https://support.moderncampus.com/troubleshooting/saxon-upgrade.html -->
		<!-- This creates a variable named PGallery that holds the data from the referenced file if it is available -->
		<!-- The contents of the external XML are stored here so that there is only one network request -->
		<xsl:variable name="PGallery">
			<xsl:if test="doc-available(concat('file:', $ou:root, $ou:site, $GalleryFileVar))">
				<xsl:copy-of select="doc(concat('file:', $ou:root, $ou:site, $GalleryFileVar))"/>
			</xsl:if>
		</xsl:variable>
				
				<xsl:for-each select="$PGallery/gallery/photo" >
		<!-- End new setup -->
			
			<xsl:variable name="thumb" select="normalize-space(thumb[text()])"/>
			<xsl:variable name="src" select="src[text()]"/>
			<xsl:variable name = "alt" select="alt[text()]" />
			<xsl:variable name="pos" select="position()" />

				<!-- thumb -->
			<xsl:choose>
				<xsl:when test="$thumb != ''">
					 <a href="javascript:void(0);" onclick="showDetail('photoDiv_{$pos}');"><img class="thumb" src="{thumb[text()]}" alt="{alt[text()]}" title="{alt[text()]}"/></a>
				</xsl:when>
				<xsl:otherwise>
					<a href="javascript:void(0);" onclick="showDetail('photoDiv_{$pos}');"><img class="thumb" src="{src[text()]}" alt="{alt[text()]}" title="{alt[text()]}"/></a>
				</xsl:otherwise>
				</xsl:choose>
		...
		<!-- New set up due to XSLT Processor Upgrade to Saxon 11.3 5/19/22 https://support.moderncampus.com/troubleshooting/saxon-upgrade.html -->
		<!-- This creates a variable named PDetail that holds the data from the referenced file if it is available -->
		<!-- The contents of the external XML are stored here so that there is only one network request -->
		<xsl:variable name="PDetail">
			<xsl:if test="doc-available(concat('file:', $ou:root, $ou:site, $GalleryFileVar))">
				<xsl:copy-of select="doc(concat('file:', $ou:root, $ou:site, $GalleryFileVar))"/>
			</xsl:if>
		</xsl:variable>
				
				<xsl:for-each select="$PDetail/gallery/photo" >
		<!-- End new setup -->
			...
			<xsl:variable name = "alt" select="alt[text()]" />
			<xsl:variable name="pos" select="position()" />
			<xsl:variable name="next" select="sum(position()+1)" />
			<xsl:variable name="prev" select="sum(position()-1)" />
			<xsl:variable name="total" select="max(/gallery/photo/position())" />
			
			<div id="photoDiv_{$pos}" class="photoDiv">
				<a name="{$pos}"><div class="center photoNav">
			<!-- prev link -->
				<xsl:choose>
				<xsl:when test="$prev=0">
					 <a href="javascript:void({$total});" onclick="visibility('photoDiv_{$pos}','photoDiv_{$total}');">&lt; Prev</a>
				</xsl:when>
				<xsl:otherwise>
					 <a name="{$pos}" href="javascript:void({$prev});" onclick="visibility('photoDiv_{$pos}','photoDiv_{$prev}');">&lt; Prev</a>
				</xsl:otherwise>
				</xsl:choose>
			<!-- gallery link -->
			| <a href="javascript:void(0);" onclick="showGallery();">Gallery</a> | 
			<!-- next link -->
				<xsl:choose>
				<xsl:when test="$next=sum($total+1)">
					 <a href="javascript:void(1);" onclick="visibility('photoDiv_{$pos}','photoDiv_1');">Next &gt;</a>
				</xsl:when>	
				<xsl:otherwise>
					 <a href="javascript:void({$next});" onclick="visibility('photoDiv_{$pos}','photoDiv_{$next}');">Next &gt;</a>	
				</xsl:otherwise>
				</xsl:choose>
			</div></a>
			...
			<div class="center ">
				<!-- photo -->
				<img class="photo" src="{src[text()]}" alt="{alt[text()]}" title="{alt[text()]}"/>
				<!-- caption -->
				<div class="photoCaption center"><xsl:value-of select="caption[text()]" /></div>
			</div>
			</div>
