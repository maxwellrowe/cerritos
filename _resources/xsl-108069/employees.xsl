            ...
            <li>
              ...
              <strong><xsl:value-of select="firstname"/></strong>, <xsl:value-of select="jobtitle"/>, <xsl:value-of select="divdept"/> 
               <xsl:if test="$phone != ''">, ext. <xsl:value-of select="$phone"/></xsl:if>
				<xsl:if test="$emailaddr != '' and substring-before(lower-case($emailaddr),'@cerritos.edu') != 'noreply'">, <a href="/scripts/employeesmailform.aspx?e={ou:parseString(substring-before($emailaddr,'@cerritos.edu'))}">
