    ...
    <xsl:template name="CerritosDirectEditButton">
        ...
        <xsl:choose>
            <xsl:when test="string-length(normalize-space($dirname2)) = 0">
                ...
                Last Update: <a href="javascript:void(0);" onclick="popupWarningEdit('wregerwgregreg&amp;account=2017&amp;site={$site1}&amp;action=de&amp;path={concat('/',ou:replaceFileExtension($filename1,'pcf'))}');"><xsl:value-of select="ou:dateFromDateTime($ou:modified,'/')" /></a>
            </xsl:when>
            <xsl:otherwise>
                Last Update: <a href="javascript:void(0);" onclick="popupWarningEdit('regregregregregreger&amp;account=2017&amp;site={$site1}&amp;action=de&amp;path={concat($dirname1,'/',ou:replaceFileExtension($filename1,'pcf'))}');"><xsl:value-of select="ou:dateFromDateTime($ou:modified,'/')" /></a>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
