      ...
      <ul class="AbcItem">
        <xsl:for-each select="document(concat('file:', $ou:root, $ou:site, '/_resources/xml/employees.xml'))/employeeslist/employee"> 
          ...
          <xsl:variable name = "emailaddr" select="emailaddr[text()]" />
			...
			<!--<xsl:variable name="firstnametemp" select="replace(replace($firstname, ' ', ''), ',','')/>-->
			<xsl:variable name="firstnametemp1" select="replace($firstname, ' ', '')"/>
			<xsl:variable name="firstnametemp2" select="replace($firstnametemp1, ',', '')"/>
			<xsl:variable name="firstnametemp3" select="replace($firstnametemp2, '.', '')"/>
				
			<xsl:variable name="middlenametemp1" select="replace($middlename, ' ', '')"/>
			<xsl:variable name="middlenametemp2" select="replace($middlenametemp1, ',', '')"/>
			<xsl:variable name="middlenametemp3" select="replace($middlenametemp2, '.', '')"/>
				
			<xsl:variable name="lastnametemp1" select="replace($lastname, ' ', '')"/>
			<xsl:variable name="lastnametemp2" select="replace($lastnametemp1, ',', '')"/>
			<xsl:variable name="lastnametemp3" select="replace($lastnametemp2, '.', '')"/>
				
			<xsl:variable name="emailtemp" select="replace(concat($firstnametemp2,$middlenametemp2,$lastnametemp2),' ','')"/>
			<xsl:if test="substring($lastname,1,1)=$seq">
            <li>
				<xsl:choose>
					<xsl:when test="$emailaddr != '' and substring-before(lower-case($emailaddr),'@cerritos.edu') != 'noreply'">
						<strong><a href="{substring-before(lower-case($emailaddr),'@cerritos.edu')}.htm"><xsl:value-of select="lastname"/>, <xsl:value-of select="firstname"/>&nbsp;<xsl:value-of select="middlename"/></a></strong>
					</xsl:when>
					<xsl:otherwise>
						<strong><a href="{lower-case($emailtemp)}.htm"><xsl:value-of select="lastname"/>, <xsl:value-of select="firstname"/>&nbsp;<xsl:value-of select="middlename"/></a></strong>
					</xsl:otherwise>
				</xsl:choose>
            </li>
		</xsl:if>
