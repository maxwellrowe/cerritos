...
<xsl:template match="/">
	...
	<xsl:variable name="firstname" select="document/ouc:properties[@label='config']/parameter[@name='firstname']/text()" />
	<xsl:variable name="lastname" select="document/ouc:properties[@label='config']/parameter[@name='lastname']/text()" />
	<xsl:variable name="middlename" select="document/ouc:properties[@label='config']/parameter[@name='middlename']/text()" />
	<xsl:variable name="divdept" select="document/ouc:properties[@label='config']/parameter[@name='divdept']/text()" />
	<xsl:variable name="jobtitle" select="document/ouc:properties[@label='config']/parameter[@name='jobtitle']/text()" />
	<xsl:variable name="phone" select="document/ouc:properties[@label='config']/parameter[@name='phone']/text()" />
	<xsl:variable name="emailaddr" select="document/ouc:properties[@label='config']/parameter[@name='emailaddr']/text()" />
	<xsl:variable name="imgurl" select="document/ouc:properties[@label='config']/parameter[@name='imgurl']/text()" />
			...
			<!-- breadcrumbs -->
               <xsl:call-template name="BreadcrumbsList"/>
				...
				<div id="contentcontainer">
					...
					<h3><xsl:value-of select="$firstname" />&nbsp;<xsl:value-of select="$middlename" />&nbsp;<xsl:value-of select="$lastname" /></h3>
					<p><xsl:value-of select="$jobtitle" /></p>
					<p><xsl:value-of select="$divdept" /></p>
					<p>Ext. <xsl:value-of select="$phone" /></p>
					<xsl:if test="$emailaddr != ''"><p><a href="/scripts/employeesmailform.aspx?e={ou:parseString(substring-before($emailaddr,'@cerritos.edu'))}">
						Email <xsl:value-of select="$firstname"/>&#160;<xsl:value-of select="$middlename" />&nbsp;<xsl:value-of select="$lastname"/> 
					</a></p></xsl:if>
					</div>
