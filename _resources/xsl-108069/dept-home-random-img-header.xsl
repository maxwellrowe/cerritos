...
<xsl:stylesheet version="3.0"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="TopPanelCSS" select="normalize-space(document/ouc:properties[@label='config']/parameter[@name='TopPanelCSS']/text())" />
		...
		<html lang="en-us">
			<head>				
				<!-- head include -->
				<xsl:copy-of select="ou:includeFile('/_resources/includes/head-2.inc')"/><xsl:text>&#xA;</xsl:text>
				...
				<!-- dept top panel styles for unique background image -->
				<xsl:comment> dept top panel styles for unique background image </xsl:comment>
				<xsl:if test="string-length($TopPanelCSS) > 0 ">
					<style type="text/css"><xsl:text>&#xA;</xsl:text>
						<xsl:value-of select="$TopPanelCSS" disable-output-escaping="yes" /><xsl:text>&#xA;</xsl:text>
					</style><xsl:text>&#xA;</xsl:text>
				</xsl:if>
				
				<!-- dept top panel script for random background image -->
				<xsl:comment> dept top panel script for random background image </xsl:comment>
	...
	<script type="text/javascript" src="//cdn.rlets.com/capture_configs/bb8/383/183/d884dfea7464d864d64e072.js" async="async"></script>
			...
			<body class="department" onload="randombg()">
				<!--######## Global Safety Alert ######## -->
				...
				<xsl:call-template name="unparsed-include-file">
					...
					<div class="clearfix">
						<div class="row">
						<!-- two column layout with left nav + main -->
						...
						<div id="maincontent" class="col-sm-8 col-md-9">
							...
							<!-- dept home with 5 edit regions or inside with 2 edit regions -->
