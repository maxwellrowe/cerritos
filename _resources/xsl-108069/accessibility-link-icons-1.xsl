	...
	<xsl:template match="a">	
		...
		<xsl:if test="$ou:action ='edt'">
			...
			<xsl:if test=".[(contains(lower-case(text()),'here') or contains(lower-case(text()),'click') 
				...
				<span class="ua-error-icon"/>
				<span class="ua-error"> 
					<a href="/public-relations/web-administrator/web-handbook/accessibility/link-text.htm" target="_blank">unclear link text: "<xsl:value-of select="text()" />"</a></span>
			</xsl:if>-->
		...
		<xsl:if test="$ou:action ='edt'">
			...
			<xsl:choose>
				<xsl:when test="./caption[string-length(normalize-space(text())) = 0]">
					<span class="ua-error-icon"/>
					<span class="ua-error"> 
						<a href="{{f:80275939}}" target="_blank">table error: no caption</a></span>
				</xsl:when>
				<xsl:otherwise>
					<span class="ua-info-icon"/>
					<span class="ua-info"> 
			...
			<xsl:choose>
				<xsl:when test=".//th[string-length(normalize-space(text())) = 0]">
					<span class="ua-error-icon"/>
					<span class="ua-error"> 
						<a href="{{f:80275939}}" target="_blank">table error: no table headers</a></span>
				</xsl:when>
				<xsl:otherwise>
