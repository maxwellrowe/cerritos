...
if (file_exists($descJsonFile)) {
    ...
    // Handle different structural formats of degree_descriptions.json (array of objects or key-value map)
    if (is_array($decoded_desc)) {
        // If it contains a nested array key like 'degrees' or 'programs'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_major') {
    ...
    if (!empty($selected_major_key)) {
        // Expected format: program_name|credential_type
        ...
        // Save to individual data/applicant_ JSON file
        file_put_contents($personalizedFilename, json_encode($applicant, JSON_PRETTY_PRINT));
        
        // Update global data/applicant__history.json file
        $historyData = [];
...
// Correct extraction logic: Iterate through degree/program objects and check if any selected career is in their related_career_matches
$matched_programs = [];

function extract_programs_from_degrees($data, $target_careers, &$results) {
    if (!is_array($data)) return;
    
    // Check if current node is a degree/program object containing program_name and related_career_matches
    if (isset($data['program_name']) && isset($data['related_career_matches']) && is_array($data['related_career_matches'])) {
        ...
        // Initial fallback description if present in degrees.json
        $prog_desc = trim($data['description'] ?? '');
        
        // Check each related career match against the user's selected careers
...
// Fallback search if no explicit matches found through related_career_matches
...
// Consolidate programs and inject descriptions strictly from degree_descriptions.json when matched
...
foreach ($matched_programs as $p) {
    ...
    // Check degree_descriptions.json lookup map first (case-insensitive)
    ...
    if (isset($degree_descriptions_lookup[$lower_name])) {
        ...
        // Fallback to internal/embedded description if not found in degree_descriptions.json
    ...
    // Avoid duplicate career entries under the same program/credential
    $exists = false;
...
// Re-index / order so Political Science comes first if present
...
<head>
    ...
    <style>
        ...
        /* Floating Overlay Tooltip Styles */
        #description-tooltip {
            position: fixed;
            z-index: 9999;
            width: 320px;
...
<script>
    // Floating tooltip logic with right-side cursor offset
    ...
    function updateTooltipPosition(event) {
        const tooltipBox = document.getElementById('description-tooltip');
        const offsetX = 20; // Distance to the right of the cursor
        const offsetY = 15; // Distance below the cursor
        
        let posX = event.clientX + offsetX;
        let posY = event.clientY + offsetY;

        // Prevent tooltip from overflowing off the right edge of the screen
        const tooltipWidth = tooltipBox.offsetWidth;
        if (posX + tooltipWidth > window.innerWidth) {
            posX = event.clientX - tooltipWidth - 10;
        }

        // Prevent tooltip from overflowing off the bottom edge of the screen
    ...
    // Helper to check the inner radio button when clicking anywhere on the program card
