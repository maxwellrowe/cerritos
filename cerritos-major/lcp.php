...
// Ensure applicant data from Page 1 exists, otherwise redirect to start[cite: 8]
if (!isset($_SESSION['applicant'])) {
    header('Location: index.php');
    exit;
}

// Handle Form Submission for Page 2 with Dynamic Routing and JSON Persistence[cite: 8]
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'step_2') {
    ...
    // Retrieve current applicant info from session[cite: 8]
    ...
    $personalizedFilename = "data/data/applicant_{$safeFirst}_{$safeLast}_{$safeEmail}.json";
    $historyFilename = "data/applicant_history.json";
    
    // Update personal file data[cite: 8]
    $personalData = [];
    ...
    // Save LCP selection and timestamp[cite: 8]
    $personalData['lcp'] = $selected_pathway;
    $personalData['last_updated'] = date('Y-m-d H:i:s');
    
    // Ensure basic fields match[cite: 8]
    ...
    // Update session data[cite: 8]
    $_SESSION['applicant'] = $personalData;
    
    // Update global data/applicant_history.json[cite: 8]
    $historyData = [];
    ...
    // Match record in history using name, email, and date of birth[cite: 8]
    $foundIndex = -1;
...
// Official Cerritos College Learning and Career Pathways list[cite: 8]
$pathways = [
    [
...
// Helper to assign clean visual icons[cite: 8]
...
<div class="container">
    <form method="POST" action="lcp.php" id="pathwayForm">
        ...
        <!-- "I'm not sure" option mapped to Exploration & Discovery -->
        ...
        <div class="pathways-grid">
            <?php foreach ($pathways as $p): 
                $pName = $p['Pathway'] ?? '';
                $pDesc = $p['Description'] ?? '';
                if (stripos($pName, 'Exploration') !== false) continue; // Handled in unsure box above
