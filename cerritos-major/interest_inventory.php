...
// Ensure data/applicant_ data exists, otherwise redirect to start[cite: 9]
...
// Check if data/applicant_ already has a saved RIASEC score / top codes in their personal file
$existing_top_codes = [];
$existing_scores = [];
...
function find_inventory_data_in_data($data, &$target_codes, &$target_scores) {
    ...
    if (isset($data['interest_inventory']['top_codes']) && is_array($data['interest_inventory']['top_codes']) && !empty($data['interest_inventory']['top_codes'])) {
        $target_codes = $data['interest_inventory']['top_codes'];
    }
    if (isset($data['interest_inventory']['scores']) && is_array($data['interest_inventory']['scores']) && !empty($data['interest_inventory']['scores'])) {
...
if (isset($_POST['action']) && $_POST['action'] === 'update_existing_primary' && $has_existing_score) {
    ...
    if (!empty($new_primary) && in_array($new_primary, $existing_top_codes)) {
        ...
        // Update persistent session and storage
        ...
        $personalData['top_three_codes'] = $existing_top_codes;
        if (isset($personalData['interest_inventory'])) {
            $personalData['interest_inventory']['top_codes'] = $existing_top_codes;
        }
...
// Handle user choice to bypass or retake
...
// Load survey questions strictly from interest_inventory.json[cite: 9]
...
// Map themes for results summary later and group questions by code[cite: 9]
$theme_lookup = [];
$grouped_by_code = [];
...
// Select 3 random questions from each code category (18 total questions across 6 categories)
if (!isset($_SESSION['selected_riasec_subset']) || isset($_GET['reshuffle'])) {
    $selected_subset = [];
    foreach ($grouped_by_code as $code => $items) {
        shuffle($items);
        $subset = array_slice($items, 0, 3); // Pick 3 per code[cite: 9]
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'resolve_tie') {
    ...
    if (!empty($primary_code) && in_array($primary_code, $current_top)) {
        // Reorder top codes so the chosen primary code comes first
        ...
        $topCodesHistory = $personalData['top_codes_history'] ?? [];
        if (!empty($topCodesHistory)) {
            $lastIdx = count($topCodesHistory) - 1;
            $topCodesHistory[$lastIdx]['top_three_codes'] = $top_code_keys;
        }
        
        $personalData['top_three_codes'] = $top_code_keys;
        if (isset($personalData['interest_inventory'])) {
            $personalData['interest_inventory']['top_codes'] = $top_code_keys;
        }
        $personalData['top_codes_history'] = $topCodesHistory;
        $personalData['last_updated'] = $currentTime;
...
// Handle Initial Survey Form Submission[cite: 9]
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_inventory') {
    ...
    // Check for ties among top scores
    ...
    $personalData['top_three_codes'] = $top_code_keys;
    $personalData['interest_inventory'] = [
        'scores' => $scores,
    ...
    <?php else: ?>
        <?php if (empty($riasec_items)): ?>
            <div class="error-notice">
                <strong>Notice:</strong> Could not load questions from <code>interest_inventory.json</code>. Please verify that the JSON file is in the same directory and formatted correctly.
            </div>
        <?php endif; ?>
