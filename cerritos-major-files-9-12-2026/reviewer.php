...
// Check for SSO / Web Server passed identities (Production Cerritos SSO headers)
...
// --------------------------------------------------------------------------
// 2. AUTHENTICATION & IDENTITY VERIFICATION FUNCTIONS
// --------------------------------------------------------------------------

/**
 * Validates user credentials against campus LDAP / Active Directory or live session endpoint.
 */
function authenticate_campus_user($username, $password) {
    if (empty($username) || empty($password)) {
        return false;
    }

    // Normalize username (extract prefix if full email provided)
    $usernameClean = strtolower(trim($username));
    if (strpos($usernameClean, '@') !== false) {
        $usernameClean = substr($usernameClean, 0, strpos($usernameClean, '@'));
    }

    $fullEmail = $usernameClean . '@cerritos.edu';

    // 1. Live Campus Authentication Check against external support/SSO validation endpoint if available
    $authEndpoint = configured_value('CAMPUS_AUTH_ENDPOINT', 'campus_auth_endpoint');
    if (!empty($authEndpoint)) {
        ...
        if ($httpCode === 200) {
            ...
            if (!empty($authData['success']) || !empty($authData['authenticated'])) {
                return $authData['email'] ?? $fullEmail;
            }
        }
    }

    // 2. Optional: Attempt live campus LDAP bind if configured/available
    $ldapServer = configured_value('LDAP_SERVER', 'ldap_server') ?? getenv('LDAP_SERVER');
    if (!empty($ldapServer) && function_exists('ldap_connect')) {
        $ds = ldap_connect($ldapServer);
        if ($ds) {
            ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, 3);
            ldap_set_option($ds, LDAP_OPT_REFERRALS, 0);
            
            $ldapDomain = configured_value('LDAP_DOMAIN', 'ldap_domain') ?? 'cerritos.edu';
            $ldapDn = "uid={$usernameClean},dc=" . str_replace('.', ',dc=', $ldapDomain);
            
            @$bind = ldap_bind($ds, $ldapDn, $password);
            ldap_unbind($ds);
            
            if ($bind) {
                return $usernameClean . '@' . $ldapDomain;
            }
        }
    }

    // 3. Fallback secure verification against local configuration hash
    $expectedPasswordHash = configured_value('REVIEWER_FALLBACK_PASSWORD_HASH', 'reviewer_password_hash');
    if (!empty($expectedPasswordHash) && password_verify($password, $expectedPasswordHash)) {
        return $fullEmail;
    }

    // Default password validation length guard for authenticated directory fallback (Local Testing)
...
// Case A: Pre-authenticated via Campus SSO / Web Server Header (Production)
if (empty($_SESSION['reviewer_authenticated']) && !empty($ssoUser)) {
    $ssoClean = strtolower(trim($ssoUser));
    if (strpos($ssoClean, '@') === false) {
        $ssoClean .= '@cerritos.edu';
    }
    $_SESSION['reviewer_authenticated'] = true;
    $_SESSION['reviewer_user'] = $ssoClean;
}

// Case B: User submits the login form directly with username and password (Local Testing fallback)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login_attempt'])) {
    $inputUser = trim($_POST['username'] ?? '');
    $inputPass = $_POST['password'] ?? '';

    $authenticatedEmail = authenticate_campus_user($inputUser, $inputPass);

    if ($authenticatedEmail !== false) {
        $_SESSION['reviewer_authenticated'] = true;
        $_SESSION['reviewer_user'] = $authenticatedEmail;
        
        header('Location: reviewer.php');
        exit;
    } else {
        $loginError = 'Invalid campus username or password. Please try again.';
    }
}

// STRICT SECURITY GATE: Enforce identity verification prior to executing any script logic
if (empty($_SESSION['reviewer_authenticated'])) {
    // Block API and GET/POST requests for unauthenticated sessions
    if ((isset($_GET['action']) && in_array($_GET['action'], ['search', 'delete', 'csv'])) || 
        ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login_attempt']))) {
        
        header('Content-Type: application/json');
        http_response_code(401);
        echo json_encode(['success' => false, 'error' => 'Unauthorized: Identity verification required.']);
        exit;
    }
    ...
    <body>
        <div class="login-card">
            ...
            <div class="sso-badge">🔒 Secure Credential Verification</div>
            <?php if ($loginError): ?>
            ...
            <form method="POST" action="reviewer.php">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="username" required placeholder="e.g. rbrammer or rbrammer@cerritos.edu">
                </div>
                <div class="form-group">
...
// Helper check for authorization whitelist & admin privileges
$currentUser = strtolower(trim($_SESSION['reviewer_user'] ?? ''));

if ($currentUser !== '' && strpos($currentUser, '@') === false) {
    $currentUser .= '@cerritos.edu';
}

/**
 * Dynamically retrieves the authorized reviewer roster.
 * Checks the live remote endpoint or falls back to the configured array.
 */
function get_authorized_reviewers() {
    $rosterEndpoint = configured_value('REVIEWER_ROSTER_ENDPOINT', 'reviewer_roster_endpoint');
    if (!empty($rosterEndpoint)) {
        $ch = curl_init($rosterEndpoint);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, !empty(private_config()['verify_google_tls']));
        $response = curl_exec($ch);
        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        ...
        if ($httpCode === 200) {
            ...
            if (is_array($data)) {
                ...
                foreach ($data as $item) {
                    if (is_string($item)) {
                        $emails[] = strtolower(trim($item));
                    } elseif (is_array($item) && isset($item['email'])) {
                        $emails[] = strtolower(trim($item['email']));
                    }
                }
                if (!empty($emails)) {
                    return $emails;
    ...
    return [
        'ctross@cerritos.edu',
        'tukita@cerritos.edu',
        'cfisher@cerritos.edu',
        'jquiroz@cerritos.edu',
        'rlee@cerritos.edu',
        'nalvarado@cerritos.edu',
        'cvandoornum@cerritos.edu',
        'rbrammer@cerritos.edu'
    ];
}

$allowedUsers = get_authorized_reviewers();
...
if (!in_array($currentUser, $allowedUsers, true)) {
    if ((isset($_GET['action']) && in_array($_GET['action'], ['search', 'delete', 'csv'])) || 
        ...
        echo json_encode(['success' => false, 'error' => 'Unauthorized: Account not authorized to access reviewer records.']);
        exit;
    }
    ...
    <head>
        <meta charset="UTF-8">
        <title>Access Denied - Student File Reviewer</title>
        <style>
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #f8fafc; color: #0f172a; display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 1rem; box-sizing: border-box; }
            .error-card { background: #fff; border: 2px solid #b91c1c; border-radius: 8px; padding: 2rem; width: 100%; max-width: 420px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); text-align: center; }
            h2 { color: #b91c1c; margin-top: 0; }
            p { color: #334155; line-height: 1.5; }
            .btn-logout { display: inline-block; margin-top: 1rem; padding: 0.75rem 1.5rem; background: #002b49; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; }
            .btn-logout:hover { background: #001f35; }
        </style>
    </head>
    <body>
        <div class="error-card">
            <h2>Access Denied</h2>
            <p>The authenticated account (<strong><?= htmlspecialchars($currentUser) ?></strong>) is not authorized to view the Student File Reviewer.</p>
            <a href="reviewer.php?action=logout" class="btn-logout">Sign Out</a>
        </div>
    </body>
    </html>
    <?php
