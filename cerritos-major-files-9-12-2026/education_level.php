...
$allowed_values = array_column($education_options, 'value');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_education') {
    ...
    // Validate submitted education option against allowed choices
    if (!in_array($goal_type, $allowed_values, true)) {
        header('Location: education_level.php?error=invalid_choice');
        exit;
    }
    ...
    // Store as education_level so summary/next_step can read it directly
    ...
    // Conditional routing logic based on goals/LCP
